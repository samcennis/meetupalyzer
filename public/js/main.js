...
$.getJSON("/meetup_data/groups.json", function(groupsJSON) {
    ...
    $.getJSON("/meetup_data/events.json", function(eventsJSON) {
        ...
        for ( var j = 0; j < top10_attended.length; j++ ){
            ...
            $('#topAverageAttendanceTable > tbody:last-child').append('<tr><td>' + groupsJSON[top10_attended[j][0]].name + '</td><td>' + groupsJSON[top10_attended[j][0]].avg_yes_rsvps_per_event_last_6_months + '</td><td>' + groupsJSON[top10_attended[j][0]].members + '</td><td>' +  (Math.round((groupsJSON[top10_attended[j][0]].avg_participation_rate * 100) * 100)/100) + '%</td></tr>');
